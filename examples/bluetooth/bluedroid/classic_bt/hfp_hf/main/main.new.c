...
void app_main(void)
{
    ...
    /* Initialize Non Volitile Storage — it is used to store PHY (Physical layer) calibration data */
    ...
    /* Enable Bluetooth Controller in Bluetooth Classic mode.  (Different than BLE Bluetooth Low Energy)*/
    if ((err = esp_bt_controller_enable(ESP_BT_MODE_CLASSIC_BT)) != ESP_OK) {
        ESP_LOGE(BT_HF_TAG, "%s enable controller failed: %s\n", __func__, esp_err_to_name(ret));
        return;
    }

    /* Run the init function for all the bluedroid helper functions and code... */
    /* TODO: No clue what all this does, looks like its prepping memory and pointers */
    /* its in the original repo under  */
    /*     esp-idf/components/bt/host/bluedroid/api/include/api/esp_bt_main.h.  */
    /*     esp-idf/components/bt/host/bluedroid/api/esp_bt_main.c               */
    if ((err = esp_bluedroid_init()) != ESP_OK) {
        ESP_LOGE(BT_HF_TAG, "%s initialize bluedroid failed: %s\n", __func__, esp_err_to_name(ret));
        return;
    }

    /* Run the enable function for all he bluedroid helper functions and code... */
    /* TODO: No clue what all this does, need to look it up */
    ...
    /* Dispatch a message that the bluetooth stack is now up and ready */
    /* This is handled below in the call -> bt_hf_client_hdl_stack_evt() */
